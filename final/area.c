...
AreaResult areaAddVoteToTribe(Area area,int area_id,int tribe_id,int num_votes){
    Area area_to_add = areaGet(area,area_id);
    assert(area_to_add!=NULL);

    int result = AREA_SUCCESS;
    char str_tribe_id[BUFFER_SIZE]="", str_num_votes[BUFFER_SIZE]="";;
    INT_TO_STR(str_tribe_id,tribe_id , result) ;
    if ( result < 0) {
        return AREA_ERROR;
    }
    ...
    if ( !mapContains(area_to_add->area_votes , str_tribe_id) ) {
        if( mapPut(area_to_add->area_votes,str_tribe_id,str_num_votes) == MAP_OUT_OF_MEMORY ){
    ...
    int new_votes = 0;
    char* str_old_vote = mapGet(area_to_add->area_votes,str_tribe_id);
...
AreaResult areaRemoveVoteFromTribe(Area area,int area_id,int tribe_id,int votes_to_remove){
    ...
    char* str_old_vote = mapGet(area_to_remove_from->area_votes,str_tribe_id);
    ...
    if ( votes_num < votes_to_remove ) {
       // votes_num = 0;
       if ( mapPut(area_to_remove_from->area_votes,str_tribe_id,NO_VOTES) == MAP_OUT_OF_MEMORY ) {
           return AREA_OUT_OF_MEMORY ;
       }
       return AREA_SUCCESS ;
    }
    ...
    if ( mapPut(area_to_remove_from->area_votes, str_tribe_id, new_vote_num) == MAP_OUT_OF_MEMORY) {
        return AREA_OUT_OF_MEMORY ;
    }
    return AREA_SUCCESS ;
}
