...
struct tribe_t { 
    Map tribe_map;
    //new struct for an option to add more data as state or political wing
};
...
TribeResult tribeAdd (Tribe tribe,int new_tribe_id , const char* tribe_name){
   ...
   result = mapPut(tribe->tribe_map ,new_tribe,tribe_name);
...
TribeResult tribeChangeName(Tribe tribe ,int tribe_id,const char* name){
    ...
    int result = TRIBE_SUCCESS;
    char new_tribe_id[BUFFER_SIZE]="";
    INT_TO_STR(new_tribe_id ,tribe_id ,result);
    if ( result < 0 ) {
       return TRIBE_ERROR;
    }
    result = mapPut(tribe->tribe_map,new_tribe_id,name);
    if (result == MAP_OUT_OF_MEMORY) {
        return TRIBE_OUT_OF_MEMORY;
    }
    return TRIBE_SUCCESS;
